...
def move_robot(socket, old_position, **kwargs):
    ...
    codified_var = np.array([[bit16rotation[1]], [bit16rotation[0]],
                             ...
                             [gripper], [0]], dtype=np.uint8).tobytes()
        
    if gripper == old_position[7,0] and np.array_equal(reached_horizontal, horizontal) and ((reached_rotation >= np.double(rotation)-10).all() and (reached_rotation <= np.double(rotation)+10).all()) and ((reached_vertical >= np.double(vertical)-10).all() and (reached_vertical <= np.double(vertical)+10).all()):
        return old_position
